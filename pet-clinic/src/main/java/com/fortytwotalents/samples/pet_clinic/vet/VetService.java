...
public class VetService {
  ...
  public VetService(
      final VetRepository vetRepository, final SpecialtyRepository specialtyRepository) {
  ...
  public List<VetDTO> findAll() {
    ...
    return vets.stream().map(vet -> mapToDTO(vet, new VetDTO())).toList();
  }
  ...
  public void update(final Integer id, final VetDTO vetDTO) {
    final Vet vet = vetRepository.findById(id).orElseThrow(NotFoundException::new);
    mapToEntity(vetDTO, vet);
  ...
  private VetDTO mapToDTO(final Vet vet, final VetDTO vetDTO) {
    ...
    vetDTO.setVetSpecialtySpecialties(
        vet.getVetSpecialtySpecialties().stream().map(specialty -> specialty.getId()).toList());
    return vetDTO;
  }
  ...
  private Vet mapToEntity(final VetDTO vetDTO, final Vet vet) {
    ...
    final List<Specialty> vetSpecialtySpecialties =
        specialtyRepository.findAllById(
            vetDTO.getVetSpecialtySpecialties() == null
                ? Collections.emptyList()
                : vetDTO.getVetSpecialtySpecialties());
