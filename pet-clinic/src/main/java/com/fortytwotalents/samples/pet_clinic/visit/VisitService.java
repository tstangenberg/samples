...
public class VisitService {
  ...
  public List<VisitDTO> findAll() {
    ...
    return visits.stream().map(visit -> mapToDTO(visit, new VisitDTO())).toList();
  }
  ...
  public void update(final Integer id, final VisitDTO visitDTO) {
    final Visit visit = visitRepository.findById(id).orElseThrow(NotFoundException::new);
    mapToEntity(visitDTO, visit);
  ...
  private Visit mapToEntity(final VisitDTO visitDTO, final Visit visit) {
    ...
    final Pet pet =
        visitDTO.getPet() == null
            ? null
            : petRepository
                .findById(visitDTO.getPet())
